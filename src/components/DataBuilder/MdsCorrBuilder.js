...
const EUC_TITLE = 'MDS display (1 - |Corr|)'
...
const MDSBuilder = ({ mode, children }) => {
  ...
  useEffect(() => {
    ...
    if (data.allMdsPointsCorrCsv.edges.length > 0) {
      const headers = Object.keys(data.allMdsPointsCorrCsv.edges[0].node)
      const content = data.allMdsPointsCorrCsv.edges
        .map(({ node }, i) => {
